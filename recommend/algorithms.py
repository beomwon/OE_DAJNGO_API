...
def crawling():
    ...
    today_foods = {'faculty_cafeteria': [], 'student_cafeteria': {'line1': [], 'line2': [], 'line3': []}}
    
    for page in range(1,3):
        search_url = f"https://www.honam.ac.kr/Cafeteria_Diet/{page}" 
        ...
        for i, item in enumerate(items):
            if today in str(item):
                today_food.append(str(list(items[i])[3])[4:-5].split(', '))

        if today_food == []:
            today_food = ['오늘의 메뉴정보가 없습니다.']
        
        if page == 1:
            today_foods['faculty_cafeteria'] = today_food[0]
        elif page == 2:
            for line in range(1, 4):
                today_foods['student_cafeteria'][f'line{line}'] = today_food[line-1]
    ...
    try:
        for team in teams:
            for user_id in team:
                ...
                # mi = {'teams':'test', 'receiver_1':'01048703170', 'menu':[{'name':'test', 'aver_rating':0}, {'name':'test', 'aver_rating':0}, {'name':'test', 'aver_rating':0}, {'name':'test', 'aver_rating':0}]}
...
● 오늘의 학식정보
(굿) 교직원식당
1. {', '.join(school_food['faculty_cafeteria'])}

(굿) 학생식당
1. {', '.join(school_food['student_cafeteria']['line1'])}
2. {', '.join(school_food['student_cafeteria']['line2'])}
3. {', '.join(school_food['student_cafeteria']['line3'])}
"""
                            }
